...
class ModelTrainer(pl.LightningModule):
    """ trainer of the reparameterization model """
    def __init__(self, hparams: Namespace, *args, **kwargs):
        super(ModelTrainer,self).__init__()
        self.save_hyperparameters(hparams)
        
        # load neural BRDF
        weight = torch.load(hparams.brdf_ckpt,map_location='cpu')
    ...
    def configure_optimizers(self,):
        ...
        optimizer = opt(self.model.parameters(),lr=self.hparams.learning_rate)
    ...
    def training_step(self, batch, batch_idx):
        ...
        brdf = self.neumip.get_rgb(f_rgb,wo,wi).relu().mean(-1).add(1e-14)
        reparam = brdf*detJ.relu() # clamp the Jacobian to enforce injectivity

        # defensive sampling 
        r2 = (1+z.pow(2).sum(-1))
        wi2,detJ2 = z/r2.unsqueeze(-1).sqrt(),1/r2.pow(2)
        brdf2 = self.neumip.get_rgb(f_rgb,wo,wi2).relu().mean(-1).add(1e-14)
        reparam2 = brdf2*detJ2

        # blend the to reparameterization to preven zero integrand
        reparam = reparam*(1-self.hparams.alpha) + reparam2*self.hparams.alpha

        # matching the integrand by log likelihood
        loss = -torch.log(reparam).mean()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        brdf = self.neumip.get_rgb(f_rgb.expand(len(wi),-1),
                                  wo.expand_as(wi),wi).relu().mean(-1)
        brdf[wi.pow(2).sum(-1)>1] = 0
        brdf = brdf.reshape(res,res)
        
        
        # binning reparameterization model samples into histogram
        hist = torch.zeros(res,res,device=wo.device)
        B=8
        for _ in range(B):
            z = torch.randn(1000_000,2,device=wo.device)
            wi = self.model(cond.expand(len(z),-1),z)
            hist += binning2d(wi[...,0],wi[...,1],res,-1,1,-1,1)
        hist /= B
        hist /= hist.max()
        brdf /= brdf.max()
        
        loss = NF.mse_loss(hist,brdf)
        image = torch.cat([hist,brdf],0)
        self.log('val/loss',loss)
        self.val_buffer[batch_idx] = image.cpu() 
        return 
    
    def on_validation_epoch_end(self,):
        # reshape validation images in to 2x8 grid
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(checkpoint_path, monitor='val/loss', save_top_k=8, save_last=True, every_n_epochs=1)
